...
# Choose year you want to access:
year = 2010
...
# If year has less 5000 entries, then use number to fit all in single page.
if getItems[0] <= 5000:
    pages = 1

# Else, divide thousands of entries divisible by 5000 to hget number of pages
# accessed.
else:
...
df = df.infer_objects()
df.to_csv(f'nyc-marathon-{year}-eventRunner.csv', index=False)
